...
contract mdtLogicV110 is Initializable, ReentrancyGuard, Context {
    ...
    mapping(address => bool) internal WHITELIST_COIN;
    ...
    function stablecoin(address _token) external view returns (bool) {
        return STABLECOIN[_token];
    }

    function setStablecoin(address[] memory _tokens, bool _status)
        external
        onlyGnosis
    {
        for (uint256 i = 0; i < _tokens.length; i++) {
            STABLECOIN[_tokens[i]] = _status;
        }
    }

    function whitelistCoin(address _token) external view returns (bool) {
        return WHITELIST_COIN[_token];
    }

    function setWhitelistCoin(address[] memory _tokens, bool _status)
        external
        onlyGnosis
    {
        for (uint256 i = 0; i < _tokens.length; i++) {
            WHITELIST_COIN[_tokens[i]] = _status;
        }
    }
    ...
    function mintCredit(
        ...
        if (!STABLECOIN[address(_token)]) {
            require(WHITELIST_COIN[address(_token)], "Not Whitelisted");
    ...
    function executeSnipe(
        ...
        costPerSnipe = ((1 ether * costPerSnipe) / 100) * _multiplier;
